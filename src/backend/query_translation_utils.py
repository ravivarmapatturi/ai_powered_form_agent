...
def get_summarization_chain(model="gpt-4o-mini"):
    llm = ChatOpenAI(model=model, temperature=0)

    summarization_prompt = ChatPromptTemplate.from_messages([
        ("system",
         "You are an Intelligent Form Agent. "
         "Summarize the form using ONLY the provided context. "
         "Do not guess or invent details. If something is missing, write 'Not Found'. "
         "Return ONLY valid JSON (no markdown)."
        ),
        ("human",
         "Form Context:\n\n{context}\n\n"
         "Return JSON with these keys exactly:\n"
         "document_type, one_line_summary, key_fields, important_entities, "
         "missing_or_unclear_fields, red_flags, short_summary.\n\n"
         "Rules:\n"
         "- key_fields must be a list of objects with keys: field, value\n"
